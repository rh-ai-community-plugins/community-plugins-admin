...
- **Helm version pinning** — plugin install and upgrade now pin the Helm chart to the version declared in the plugin's `plugin.yaml`, preventing deployment of untested pre-release charts.
- **Charter registry URL** — default registry URL now points to the `main` branch of the charter repo instead of `dev`.

## [0.1.1] - 2026-07-20

### Fixed

- **Catalog page** — hide "Unable to determine installed plugin status" warning for non-admin users who lack RBAC to query deployments.
- **Installed page** — show a friendly "Access restricted" message instead of a raw 403 error for non-admin users.
