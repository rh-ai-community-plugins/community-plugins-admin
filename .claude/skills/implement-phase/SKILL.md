...
- If on `main` or `dev`: switch to `dev`, pull latest, then create and checkout a new feature branch from `dev` with a descriptive name (e.g., `feat/phase-1-foundation`, `feat/catalog-page`, `feat/bff-catalog`). `main` is the release branch — never branch from it for feature work. Use the phase content to pick a meaningful name.
